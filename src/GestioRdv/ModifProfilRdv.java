...
public class ModifProfilRdv {
	...
	public static void main(String[] args, Integer id) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ModifProfilRdv window = new ModifProfilRdv(id);
	...
	private void initialize(int id) {
		...
		btnModifier.addMouseListener(new MouseAdapter() {
	        ...
	        if (phone.getText().trim().matches(regexStr) && phone.getText().length() == 10) {
            	if (email.contains("@")) {
            Manager.GestioRdvManager.ModifierProfilRdv(lastname.getText(), firstname.getText(), mail.getText(), password.getText(), phone.getText(), id);
            //frame.dispose();
            }
                else {
